...
def private(request, username):
    ...
    if request.method == "GET":
        ...
        if not request.user.is_authenticated:
            ...
            if not room:
                room = Room(group_type=GroupTypes.Private, name=private_room_name).save()

            # Get the messages
            websocket_url_json = mark_safe(json.dumps(f"ws://{{0}}/OB/private/{username}/"))
            messages = Message.objects.filter(room=room)
            messages_timestrings = [(message, get_datetime_string(message.timestamp)) for message in messages]
